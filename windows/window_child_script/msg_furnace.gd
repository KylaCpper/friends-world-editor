...
func _on_pressed0() -> void:
	for i in range(99):
		if !i in Overall.furnace:
			Overall.furnace[i] = {"name":"新分类","data":[]}
			_update_tree()
			break
...
func _on_pressed2() -> void:
	if key != null:
		if key in Overall.furnace:
			Overall.furnace.erase(key)
			_update_tree()
func _on_pressed3() -> void:
	if key != null:
		if key in Overall.furnace:
			var d= {"export":[["",1,100,false]],"table":[["",1]],"note":"配方"+str(Overall.furnace[key].data.size()),"energy":1.0,"time":1.0}
			Overall.furnace[key].data.append(d)
			_update_tree()
...
func _input(event) ->void:
	...
	if event.is_action_pressed("ctrl+d"):
		if key != null:
			if key in Overall.furnace:
				Overall.furnace[key].data.append(Overall.furnace[key].data[index].duplicate(true))
				_update_tree()
